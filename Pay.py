        ...
        if len(paymentPerson)==0:# or data['recla']:
            ...
            for c in consultantList:
                ...
                if active:
                    ...
                    dfFees = dfFees.dropna(subset=['تاریخ صدور بیمه نامه'])
            ...
            df['period'] = data['period']['Show']
            df['username'] = str(username)
            df['date'] = timedate.toDaySlash()
            df = df.to_dict(orient='records')
            pishkarDb['paymentPerson'].delete_many({'period':data['period']['Show'],'username':username})
            pishkarDb['paymentPerson'].insert_many(df.copy())
            df = pd.DataFrame(df)
            df = df.drop(columns=['period','username'])
            ...
            df = df.drop(columns=['period','username'])
            df = df.to_dict(orient='records')
        return json.dumps({'replay':True,'pay':df})
    else:
        return ErrorCookie()
...
def perforator(data):
    ...
    if user['replay']:
        ...
        df = df.dropna(subset=['تاریخ صدور بیمه نامه'])
